import { filterBySearchTerm, sortData, paginateData, calculatePagination, evaluateConditions, getNestedValue, validateField, validateForm, setNestedValue, getRowKey } from './chunk-ZYRLE26I.mjs';
...
function useServerTableData(options) {
  ...
  const setPage = useCallback((p) => setPageState(Math.max(1, p)), []);
  ...
  const setSearchTerm = useCallback((term) => setSearchTermState(term), []);
  const setAdvancedFiltersCallback = useCallback((filters) => setAdvancedFilters(filters), []);
  const addFilter = useCallback((filter) => setAdvancedFilters((prev) => [...prev, filter]), []);
  const removeFilter = useCallback((index) => setAdvancedFilters((prev) => prev.filter((_, i) => i !== index)), []);
...
export { useColumnConfig, useFormState, usePaginatedData, useServerTableData, useTableSelection };
//# sourceMappingURL=chunk-7FJC5PAY.mjs.map
//# sourceMappingURL=chunk-7FJC5PAY.mjs.map