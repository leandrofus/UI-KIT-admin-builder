...
function createFormAdapter(config = {}) {
  ...
  const toFormValues = (data) => {
    ...
    for (const [fieldName, transformer] of Object.entries(fieldTransformers)) {
      const value = chunkBHNUSCJS_js.getNestedValue(result, fieldName);
      if (value !== void 0) {
        result = chunkBHNUSCJS_js.setNestedValue(result, fieldName, transformer(value, result));
      }
    }
    ...
    for (const field of fields) {
      if (field.computed && field.computed.formula) {
        ...
        const hasAllDeps = deps.every((dep) => {
          const value = chunkBHNUSCJS_js.getNestedValue(result, dep);
  ...
  const getVisibleValues = (data, fields) => {
    ...
    for (const field of fields) {
      ...
      if (field.showWhen && !chunkBHNUSCJS_js.evaluateConditions(field.showWhen, data)) {
        continue;
      }
      const value = chunkBHNUSCJS_js.getNestedValue(data, field.name);
      if (value !== void 0) {
...
function evaluateFormula(formula, data, deps) {
  ...
  for (const dep of deps) {
    const value = chunkBHNUSCJS_js.getNestedValue(data, dep);
...
//# sourceMappingURL=chunk-A7QXEUVF.js.map
//# sourceMappingURL=chunk-A7QXEUVF.js.map