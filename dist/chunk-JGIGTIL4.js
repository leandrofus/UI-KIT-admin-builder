...
function usePaginatedData(options = {}) {
  ...
  const processedData = react.useMemo(() => {
    ...
    if (searchTerm && searchableColumns.length > 0) {
      result = chunkG4EIC5OB_js.filterBySearchTerm(result, searchTerm, searchableColumns);
    }
    if (sortColumn && sortDirection) {
      result = chunkG4EIC5OB_js.sortData(result, sortColumn, sortDirection);
    }
    return result;
  ...
  const paginatedData = react.useMemo(() => {
    ...
    return chunkG4EIC5OB_js.paginateData(processedData, page, pageSize);
  }, [processedData, page, pageSize, isServerSide, rawData]);
  const paginationMeta = react.useMemo(
    () => chunkG4EIC5OB_js.calculatePagination(total, page, pageSize),
    [total, page, pageSize]
  );
  react.useEffect(() => {
...
function useServerTableData(options) {
  const {
    fetchFn,
  ...
  const [sort, setSortState] = react.useState(defaultSort);
  ...
  const fetchData = react.useCallback(async () => {
    const params = {
      ...
      searchTerm: debouncedSearchTerm || void 0,
      advancedFilters: advancedFilters.length > 0 ? advancedFilters : void 0,
      filters: advancedFilters.length > 0 ? { advanced: advancedFilters } : void 0
    };
    if (abortRef.current) {
  ...
  const setPage = react.useCallback((p) => setPageState(Math.max(1, p)), []);
  ...
  const setSearchTerm = react.useCallback((term) => setSearchTermState(term), []);
  const setAdvancedFiltersCallback = react.useCallback((filters) => setAdvancedFilters(filters), []);
  const addFilter = react.useCallback((filter) => setAdvancedFilters((prev) => [...prev, filter]), []);
  const removeFilter = react.useCallback((index) => setAdvancedFilters((prev) => prev.filter((_, i) => i !== index)), []);
...
function useFormState(options) {
  ...
  const isFieldVisible = react.useCallback((field) => {
    ...
    return chunkG4EIC5OB_js.evaluateConditions(field.showWhen, values);
  }, [values]);
  const getVisibleFields = react.useCallback((formSections) => {
    return formSections.filter((section) => {
      if (!section.showWhen) return true;
      return chunkG4EIC5OB_js.evaluateConditions(section.showWhen, values);
  ...
  const validateSingleField = react.useCallback((name) => {
    ...
    const value = chunkG4EIC5OB_js.getNestedValue(values, name);
  ...
  const validateAllFields = react.useCallback(() => {
    ...
    const result = chunkG4EIC5OB_js.validateForm(values, visibleFields);
  ...
  const setValue = react.useCallback((name, value) => {
    setValues((prev) => chunkG4EIC5OB_js.setNestedValue(prev, name, value));
    if (onChange) {
      const newValues = chunkG4EIC5OB_js.setNestedValue(values, name, value);
  ...
  const getFieldProps = react.useCallback((name) => {
    return {
      name,
      value: chunkG4EIC5OB_js.getNestedValue(values, name),
...
function useTableSelection(options = {}) {
  ...
  const [selectedMap, setSelectedMap] = react.useState(() => {
    ...
    initialSelection.forEach((row) => {
      const key = chunkG4EIC5OB_js.getRowKey(row, rowKey);
      map.set(key, row);
    });
    return map;
  });
  ...
  const getKey = react.useCallback((row) => {
    return chunkG4EIC5OB_js.getRowKey(row, rowKey);
  }, [rowKey]);
...
exports.useServerTableData = useServerTableData;
exports.useTableSelection = useTableSelection;
//# sourceMappingURL=chunk-JGIGTIL4.js.map
//# sourceMappingURL=chunk-JGIGTIL4.js.map